...
import { CODEPLUG_SCHEMA_VERSION, emptyCodeplug } from '../models/codeplug.ts';
...
describe('codeplugStorage', () => {
  ...
  it('migrates v1 codeplugs with tgLists to rxGroupLists', () => {
    ...
    const state = deserializeProjects(json);
    expect(state?.projects[0].codeplug.meta.schemaVersion).toBe(CODEPLUG_SCHEMA_VERSION);
  ...
  it('migrates v11 combined channel name to callsign and qualifier with exportNameMode', () => {
    const v11 = {
      channels: [
        {
          id: 'ch-1',
      ...
      contacts: [],
      rxGroupLists: [],
      meta: { schemaVersion: 11, importedAt: null, sourceFiles: [] },
    };
    const cp = migrateCodeplug(v11);
  ...
  it('migrates v12 talk group without abbreviation', () => {
    const v12 = {
      channels: [],
      zones: [],
      talkGroups: [{ id: 'tg-1', name: 'Scotland', number: '950', timeslotOverride: '' }],
      contacts: [],
      rxGroupLists: [],
      meta: { schemaVersion: 12, importedAt: null, sourceFiles: [] },
    };
    const cp = migrateCodeplug(v12);
    expect(cp?.talkGroups[0]).toMatchObject({
      id: 'tg-1',
      name: 'Scotland',
      number: '950',
    });
  ...
  it('preserves talk group abbreviation when present in v12 payload', () => {
    ...
    const cp = migrateCodeplug(v12);
    expect(cp?.talkGroups[0].abbreviation).toBe('Scot');
    expect(cp?.meta.schemaVersion).toBe(CODEPLUG_SCHEMA_VERSION);
  });
